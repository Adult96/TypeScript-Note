...
function assertion2(x: string | number) {
  // return x + 1; //에러 발생 유니온 타입으로 정의 해서 에러남

  let arr: number[] = [];
  arr[0] = x as number; //어썰션(assertion) 사용 왼쪽에있는 것을 오른쪽 타입으로 덮음
}
...
function 숙제_4_1(arr: (number | string)[]) {
...
function 숙제_4_2(obj: { subject: string | string[] }) {
